...
//        iArr3 = new int[][];      컴파일 에러 발생
        ...
        for(int i = 0; i< iArr1.length; i++){           // 1차원 배열을 고르는 for 문
            for(int j = 0; j < iArr1[i].length; j++){   // 선택된 2차원 배열의 길이만큼 반복하는 for문
                iArr1[i][j] = num++;
            }
        ...
        int length = 1;
        for(int i = 0; i< iArr2.length; i++){           // 1차원 배열을 고르는 for 문
            iArr2[i] = new int[++length];
            for(int j = 0; j < iArr2[i].length; j++){   // 선택된 2차원 배열의 길이만큼 반복하는 for문
                iArr2[i][j] = num++;
            }
