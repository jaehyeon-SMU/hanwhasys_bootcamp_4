...
public class RequestHeaderPrintServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        /* 설명.
        *   요청 시 전달되는 헤더라는 것이 어떤 정보들을 가지고 있는지 확인해 보자
        ...
        *   2. Request Header:
        *       패치될 리소스나 클라이언트 자체에 대한 상세 정보를 포함하는 헤더이다.
        *   3. Response Header:
        *       요청 위치나 서버 응답에 대한 부가적인 정보를 갖는 헤더이다.
        *   4. Entity Header:
        *       컨텐츠 길이나 MIME 타입과 같은 엔티티 바디에 대한 상세정보를 포함하는 헤더이다.
        *       (요청 및 응답 모두에서 사용되며, 메시지 body의 컨텐츠를 나타내기에 GET요청은 해당되지 않는다.)
        *       (Content_Length, Content-Type, Content-Language, Content- Encoding)     // body에 대한 내용을 header로 표현
        * */
