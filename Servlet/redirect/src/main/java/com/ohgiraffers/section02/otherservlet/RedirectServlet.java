...
public class RedirectServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        /* 설명. 리다이렉트 이후 오는 서블릿은 기존과 다른 새로운 request아 response 객체로 오게 된다 */
        System.out.println("리다이렉트 이후 request의 atrribute: " + req.getAttribute("test")); //null

        /* 설명. 파라미터가 넘어오면 request 객체가 달라져도 값이 전달되어 온다. */
        System.out.println("리다이렉트 이후 request의 parameter: " + req.getParameter("test"));

        /* 설명.
        *   redirect 시에는 request의 attribute 개념으로 다른 서블릿에게 값(상태)을 전달 할 수 없다.
        *   왜냐하면 다음 서블릿으로 클라이언트가 새로 요청하면 기존의 request와 response 객체가 아닌
        *   새로운 request와 response 객체가 만들어지며 요청하기 때문이다.
        *
        *   설명
        *     추가로,cookie와 session같은 공간을 활용하는 방법을 사용하여 값을 공유할 수도 있다.
        * */
