...
*   (불변 객체: setter 가 없고 필드값에 변화를 주고 싶으면 새로운 필드를 가지는 새로운 객체가 생성되어야 한다.)
*   그리고 equals,hashCode를 반드시 오버라이딩 해야한다.
*    VO와 유사하며 @EmbeddedId로 복합키를 표현하고자 할 때 쓰이기도 한다.
* */
@Embeddable
public class MemberPK implements Serializable {     // embedded 타입은 Serializable을 무조건 implement 해줘야 된다.
