...
@Configuration("configurationSection03")        // 자신이 있는 패키지만 스캔한다.

/* 설명. 1. 단순히 범위 지정만으로 @Component 계열의 어노테이션들을 모두 bean으로 관리하는 경우 */
//@ComponentScan(basePackages = "com.ohgiraffers")        // 콩이 어디있는지 스캔할 범위 설정(어노테이션이 달려있으면 콩으로 인식)
...
@ComponentScan(basePackages = {"com.ohgiraffers","com"},        //여러 패키지도 설정 가능
...
/* 설명.
*   @ComponentScan은 필터의 경우 개념 정도만 알아도 되지만 기본적으로 @Conifuration 클래스가 위치한
