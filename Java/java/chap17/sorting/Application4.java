...
public class Application4 {
    ...
    private static int process(int left, int right, int[] arr) {  //현재 정렬 범위에서 피벗을 찾고, 배열을 피벗을 기준으로 정렬
        int lo = left -1;           // 0번인덱스 부터 시작하기위해(do -while문에서 lo++하기 때문) left-1 즉 0번 인덱스보다 더 왼쪽에서 시작
        int hi = right + 1;
