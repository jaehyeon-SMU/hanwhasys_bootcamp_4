...
CREATE OR REPLACE TRIGGER after_order_menu_insert  -- after_order_menu_insert가 이름이다.
    AFTER INSERT
    ON tbl_order_menu -- 내역 테이블
    FOR EACH ROW
BEGIN
    UPDATE tbl_order  -- 이력 테이블
...
-- 주문 테이블(tbl_order)에 insert후 주문 메뉴 테이블(tbl_order_menu)에 주문한 메뉴마다
